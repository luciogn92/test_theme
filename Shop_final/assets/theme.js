...
theme.Slideshow = (function() {
  ...
  function Slideshow(container, options) {
    ...
    {
      this.buttons = document.querySelectorAll(".slider--" + this.container.getAttribute('data-section-id') + selectors.button);
  ...
  Slideshow.prototype = Object.assign({}, Slideshow.prototype, {
    ...
    destroy: function() {
      ...
      if ( this.options && this.options.type === 'slide') {
        window.removeEventListener(
          'resize',
          this.eventHandlers.debounceResizeSlideIn
        );

        if (this.touchEvents && this.options && this.options.canUseTouchEvents) {
    ...
    _setupSlideType: function(onResize) {
      ...
      this.slides.forEach(function(sliderItem, index) {
        ...
        this.sliderItemWidthTotal =
          this.sliderItemWidthTotal + this.sliderItemWidth;
    ...
    _onTouchMove: function(event, direction, difference) {
      ...
      if (
        ...
        ((event.clientX <= threshold ||
          event.clientX >= window.innerWidth - threshold) || (event.clientY <= threshold || event.clientY >= window.innerHeight - threshold))
      ) {
      ...
      if ( (!this.options.vertical) && direction !== 'left' && direction !== 'right' ) return;
      if (this.options.vertical && direction !== 'up' && direction !== 'down' ) return;

      if (!this.options.vertical)
      {this.touchMovePosition = this.touchStartPosition + difference.xPosition;}
      else
      {this.touchMovePosition = this.touchStartPosition + difference.yPosition;}
    ...
    _onTouchEnd: function(event, direction, difference) {
      ...
      if (this.options.vertical)
      {var slideDirection = direction === 'up' ? 'next' : '';}
      else
    ...
    _isNextTranslateXLast: function(translateXValue) {
      ...
      var nextTranslateXValue =
        translateXValueAbsolute + (this.options.slidesToShow * this.sliderItemWidth);
...
theme.Product = (function() {
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _adjustThumbnailSlider: function(activeThumbnail) {
      ...
      window.setTimeout(
        function() {
          if (!(this.slideshow) || !(this.slideshow.container) ) return;
    ...
    _switchMedia: function(mediaId) {
      var currentMedia = this.container.querySelector(
        this.selectors.productMediaWrapper +
          ':not(.' +
          this.classes.hidden +
          ')'  + ":not(." + this.classes.no_thumbnail + ")"
      );

      var newMedia = this.container.querySelector(
        this.selectors.productMediaWrapper + "[data-media-id='" + mediaId + "']" + ":not(." + this.classes.no_thumbnail + ")"
      );
...
theme.CustomSlider = (function() {
  ...
  function CustomSlider(container) {
    ...
    this.container = container;
    var sectionId = container.getAttribute('data-section-id');
    this.slider = document.getElementById('CustomSlider-' + sectionId);

    let extraOptions = this.slider.getAttribute('data-extra-options');
    if(extraOptions)
    {
      try {
        this.extraOptions = JSON.parse('{' + extraOptions + '}');
      } catch (error) {
        console.log(error);
      }
    }
    ...
    this.desktopOptions = Object.assign({}, options, {
      slidesToShow: Math.min(
    ...
    this.initSlider = this._initSlider.bind(this);

    this.initSlider();
  }

  CustomSlider.prototype = Object.assign({}, CustomSlider.prototype, {
    onUnload: function() {
      this.slideshow.destroy();
    },
    ...
    onBlockSelect: function(evt) {
      var slide = document.querySelector(
        '.customSlider-slide--' + evt.detail.blockId
      );
      var slideIndex = Number(slide.getAttribute('data-slider-slide-index'));
    ...
    _initSlider: function() {
        this._initInstanceSlider(this.desktopOptions);
    },

    // eslint-disable-next-line no-unused-vars
    _initInstanceSlider: function(args) {
      if (this.sliderActive) {
        this.slideshow.destroy();
        this.sliderActive = false;
      }
      ...
      if(theme.customSliders)
      {
        theme.customSliders[this.slider.id] = this.slideshow;
      }
      else
      { 
        theme.customSliders = {};
      ...
      this.container.dispatchEvent(new CustomEvent('customSlider_ready', {
        bubbles: true,
        cancelable: true
      }));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  sections.register('customSlider', theme.CustomSlider);
